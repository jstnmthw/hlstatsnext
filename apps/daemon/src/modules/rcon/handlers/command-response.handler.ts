...
export class CommandResponseHandler {
  ...
  private extractResponseText(buffer: Buffer): string {
    ...
    if (buffer.length > 5 && buffer[0] === 0xFF && buffer[1] === 0xFF && buffer[2] === 0xFF && buffer[3] === 0xFF) {
    ...
    this.logger.warn("⚠️ GoldSrc RCON: Non-standard response format detected")
  ...
  private checkForErrors(response: string, command: string): RconError | null {
    ...
    if (response.includes("Unknown command")) {
      this.logger.warn(`⚠️ GoldSrc RCON: Unknown command: ${command}`)
