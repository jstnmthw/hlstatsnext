...
export class MatchRepository extends BaseRepository<ServerRecord> implements IMatchRepository {
  ...
  async updateServerStats(serverId: number, updates: Record<string, unknown>, options?: UpdateOptions): Promise<void> {
    try {
  ...
  async createPlayerHistory(data: PlayerHistoryData, options?: CreateOptions): Promise<void> {
    try {
      ...
      await this.executeWithTransaction(async (tx) => {
        const table = options?.transaction ? (tx as any).playerHistory : this.db.prisma.playerHistory
        await table.create({
          data: {
  ...
  async updateTeamWins(serverId: number, winningTeam: string, options?: UpdateOptions): Promise<void> {
    try {
  ...
  async updateBombStats(serverId: number, eventType: 'plant' | 'defuse', options?: UpdateOptions): Promise<void> {
    try {
      const updates: Record<string, unknown> = {}
