...
export function calculateKillSkillAdjustment(
  ...
  // Apply rating bounds
  const finalKillerChange = applyRatingBounds(
    killer.rating,
    killerGain,
    cfg.minRating,
    cfg.maxRating,
  )
  const finalVictimChange = applyRatingBounds(
    victim.rating,
    victimLoss,
    cfg.minRating,
    cfg.maxRating,
...
export function calculateStandardRatingAdjustment(
  ...
  const boundedWinnerGain = applyRatingBounds(
    winner.rating,
    winnerGain,
    cfg.minRating,
    cfg.maxRating,
 ...
 */
export function calculateSuicidePenalty(config: Partial<SkillCalculationConfig> = {}): number {
  const cfg = { ...DEFAULT_SKILL_CONFIG, ...config }
 ...
 */
export function calculateTeamKillPenalty(config: Partial<SkillCalculationConfig> = {}): {
  killerChange: number
  victimChange: number
} {
  const cfg = { ...DEFAULT_SKILL_CONFIG, ...config }
