...
export class HLStatsDaemon {
  ...
  async stop(): Promise<void> {
    ...
    try {
      ...
      this.logger.failed('Error during shutdown', error instanceof Error ? error.message : String(error))
    }
  }
  ...
  private async testDatabaseConnection(): Promise<boolean> {
    try {
      ...
      this.logger.failed('Database connection test failed', error instanceof Error ? error.message : String(error))
      return false
    }
  }
  ...
  private async disconnectDatabase(): Promise<void> {
    try {
      ...
      this.logger.failed('Error closing database connection', error instanceof Error ? error.message : String(error))
    }
  }
}
