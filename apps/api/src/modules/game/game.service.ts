...
import type { GameStatistics, CreateGameInput, UpdateGameInput } from "./game.types"
import type { Result, AppError } from "@/shared/types"
import { ACTIVITY_TIMEFRAMES } from "./game.types"
...
export class GameService {
  ...
  async getGameStats(gameId: string): Promise<Result<GameStatistics, AppError>> {
    try {
      ...
      const thirtyDaysAgo = Math.floor(Date.now() / 1000) - ACTIVITY_TIMEFRAMES.THIRTY_DAYS

      const [totalPlayers, activePlayers, stats, topPlayers] = await Promise.all([
        // Total players
   ...
   */
  async updateGame(code: string, input: UpdateGameInput): Promise<Result<Game, AppError>> {
    try {
