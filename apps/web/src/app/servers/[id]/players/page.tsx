...
export async function generateMetadata({ params }: ServerPlayersPageProps): Promise<Metadata> {
  ...
  try {
    ...
    return {
      title: server?.name
        ? `${server.name} - Player Rankings`
        : `Server #${serverId} - Player Rankings`,
      description: server
        ? `Player leaderboard and statistics for ${server.name} (${server.address}:${server.port})`
        : `Player leaderboard for server #${serverId}`,
    }
  } catch {
    return {
      title: "Server Player Rankings",
      description: "View server player leaderboard and statistics",
    }
  }
}

export default async function ServerPlayersPage({ params, searchParams }: ServerPlayersPageProps) {
  const { id } = await params
  ...
  const urlParams = parseUrlParams(searchParamsData as Record<string, string | undefined>, {
    sortField: "skill",
    sortOrder: "desc",
    pageSize: 50, // Show more players on dedicated page
  })
          ...
          <div>
            <div className="mb-6">
              ...
              <p className="text-muted-foreground mt-2">
                Complete leaderboard showing all players who have played on this server, ranked by
                skill rating and performance statistics.
              </p>
            </div>
