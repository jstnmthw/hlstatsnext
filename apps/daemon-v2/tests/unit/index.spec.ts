...
describe("HLStatsDaemon", () => {
  ...
  describe("start", () => {
    it("should start all services successfully", async () => {
      ...
      const processorInstance = vi.mocked(EventProcessorService).mock
        .instances[0] as Mocked<EventProcessorService>;
      processorInstance.testDatabaseConnection.mockResolvedValue(true);

      const gatewayInstance = vi.mocked(GatewayService).mock
        .instances[0] as Mocked<GatewayService>;
      gatewayInstance.start.mockResolvedValue(undefined);

      const ingressInstance = vi.mocked(IngressService).mock
        .instances[0] as Mocked<IngressService>;
      ingressInstance.start.mockResolvedValue(undefined);

      const rconInstance = vi.mocked(RconService).mock
        .instances[0] as Mocked<RconService>;
