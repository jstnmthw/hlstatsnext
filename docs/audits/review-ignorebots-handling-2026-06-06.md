...
- [x] **Inconsistent `IgnoreBots` defaults across call sites** — most readers defaulted to `true` (`connect-event.handler.ts`, `player.events.ts`, `player-status-enricher.ts`, `weapon.events.ts`, `player-session.service.ts`) but `server-status-enricher.ts` defaulted to `false`. **FIXED 2026-06-06.**
      **Fix applied:** Added a single typed accessor `ServerService.isIgnoreBotsEnabled(serverId)` and routed all six call sites through it, removing the duplicated `"IgnoreBots"` parameter string and the divergent `true`/`false` fallbacks. The accessor's last-resort default is centralized in one `IGNORE_BOTS_DEFAULT = false` constant chosen to **match the seeded `servers_config_default` row** (`IgnoreBots = '0'`, see `packages/db/src/sql/servers-config.sql`) — so an unseeded DB now behaves identically to a seeded one instead of silently flipping the default, and scoring/presence/session-sync/player-count enrichment can no longer disagree. Note this is only the code-level fallback: the repository's `getServerConfig` still consults the per-server `servers_config` row and the `servers_config_default` row first, so on a normal (seeded) DB nothing changes. Tests updated to stub/assert the new accessor; added direct coverage for it in `server.service.test.ts`.
